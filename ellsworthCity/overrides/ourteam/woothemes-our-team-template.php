 ...
 */
/* mods
* 29jun16 zig - allow email to show.
*/
			...
			foreach ( $query as $post ) {
				...
				if ( true == $args['display_additional'] ) {
					...
					if ( true == $args['contact_email'] && '' != $post->contact_email && apply_filters( 'woothemes_our_team_member_contact_email', true ) ) {
						// zig out $member_fields .= '<li class="our-team-contact-email" itemprop="email"><a href="mailto:' . esc_html( $post->contact_email ) . '">' . __( 'Email ', 'our-team-by-woothemes' ) . get_the_title() . '</a></li>';
						$member_fields .= '<li class="our-team-contact-email" itemprop="email"><a href="mailto:' . esc_html( $post->contact_email ) . '">' . esc_html( $post->contact_email ) . '</a></li>';
					}
